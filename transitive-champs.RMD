---
title: "Identifying all the 'Transitive Champions' from the college basketball season"
...
for(x in 1:rounds){
  ...
  wbb_degree_sep <- rbind(wbb_degree_sep, wbb_t_champs %>% length)
}
...
for(x in 1:rounds){
  ...
  mbb_degree_sep <- rbind(mbb_degree_sep, mbb_t_champs %>% length)
}
...
* Women's Basketball: __`r transitive_champs %>% filter(sport == "WBB") %>% pull(transitive_champions) %>% max()` transitive champions__

* Men's Basketball: __`r transitive_champs %>% filter(sport == "MBB") %>% pull(transitive_champions) %>% max()` transitive champions__
